...
// Steering behaviors (design doc layer 7): pure force-producing functions — arrive,
// flee, wander, avoid-edges — composed by the arbiter's active behavior and fed
...
struct SteeringTests {
    ...
    /// From rest, outside the slow radius, arrive wants the full `maxSpeed` — so the
    /// steering force is exactly `maxSpeed × steeringGain` on a unit-mass body.
    @Test func arrive_farFromTarget_wantsFullSpeedForce() {
        let arrive = Steering.arrive(
            position: origin, velocity: still, target: Point(x: 2000, y: 400), maxSpeed: 100
        )
        #expect(abs(arrive.magnitude - 100 * MindConstants.steeringGain) < 0.001)
        #expect(arrive.dx > 0)
        #expect(abs(arrive.dy) < 0.001)
    }

    @Test func arrive_insideSlowRadius_wantsLessThanFullSpeedForce() {
        ...
        #expect(arrive.magnitude < 100 * MindConstants.steeringGain)
        #expect(arrive.magnitude > 0)
    }
