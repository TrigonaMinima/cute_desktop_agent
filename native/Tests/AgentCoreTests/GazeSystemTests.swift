...
// smooth pursuit. A reflex snap bypasses all hysteresis. Time convention matches the
// rest of the core: `now` in ms, `dt` in seconds.
struct GazeSystemTests {
    ...
    private func simulate(
        _ gaze: inout GazeSystem, context: GazeContext, from: Double, seconds: Double
    ) -> Double {
        let dt = 1.0 / 60.0
        var now = from
        for _ in 0..<Int(seconds * 60) {
            now += dt * 1000
            gaze.update(context: context, now: now, dt: dt)
        }
        return now
    }
    ...
    @Test func update_quietWorld_staysNeutral() {
        ...
        simulate(&gaze, context: makeContext(world: makeWorld()), from: 0, seconds: 2)
        #expect(gaze.targetKind == .neutral)
    }
    ...
    @Test func update_fastNearbyCursor_capturesGaze() {
        ...
        simulate(&gaze, context: context, from: 0, seconds: 1)
    ...
    @Test func update_windowOnset_capturesGaze() {
        ...
        let now = simulate(
            &gaze, context: makeContext(world: makeWorld(frontmostWindow: windowA)),
            from: 0, seconds: 1
        )
        simulate(
            &gaze, context: makeContext(world: makeWorld(frontmostWindow: windowB)),
    ...
    @Test func update_onsetAttendedLong_releasesToTheUser() {
        ...
        now = simulate(&gaze, context: contextB, from: now, seconds: 0.5)
        #expect(gaze.targetKind == .onset)
        simulate(&gaze, context: contextB, from: now, seconds: 30)
    ...
    @Test func snap_bypassesMarginAndDwell_immediately() {
        var gaze = GazeSystem(bodyCenter: bodyCenter, now: 0)
        gaze.update(context: makeContext(world: makeWorld()), now: 16, dt: 1.0 / 60.0)
    ...
    @Test func update_justAfterSwitch_saccadeClosesMostOfTheGapFast() {
        ...
        var now = simulate(&gaze, context: context, from: 0, seconds: 0.4)
        #expect(gaze.targetKind == .cursor)
        let initialGap = distance(gaze.gazePoint, Point(x: 650, y: 400))
        now = simulate(&gaze, context: context, from: now, seconds: 0.15)
    ...
    @Test func update_higherAttention_tracksTighterThanLower() {
        ...
        var drowsy = locked
        simulate(&locked, context: context, from: 0, seconds: 1)
        simulate(&drowsy, context: context, from: 0, seconds: 1)
        ...
        for _ in 0..<60 {
            nowA += dt * 1000
            nowB += dt * 1000
            locked.update(context: moved, now: nowA, dt: dt)
            drowsy.update(context: moved, now: nowB, dt: dt)
            locked.attention = 1.0
            drowsy.attention = 0.1
        }
